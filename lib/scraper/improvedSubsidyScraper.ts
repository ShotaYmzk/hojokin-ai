...
const IMPROVED_TARGETS: ImprovedScrapingTarget[] = [
  {
    ...
    selectors: {
      ...
      detailContent: '.subsidy-overview, .description, .detail-content, .summary, .content',
      detailAmount: '.amount-info, .subsidy-amount, .budget-info, table td:contains("上限額"), table td:contains("補助額")',
      detailDeadline: '.deadline-info, .application-period, table td:contains("募集期間"), table td:contains("申請期限")',
      detailOrganization: '.organization-info, .sponsor-info, table td:contains("実施機関"), table td:contains("主催")',
...
export class ImprovedSubsidyScraper {
  ...
  private async scrapeImprovedTarget(target: ImprovedScrapingTarget): Promise<void> {
  ...
  private async parseDetailPage($: ReturnType<typeof cheerio.load>, target: ImprovedScrapingTarget): Promise<any> {
  ...
  private extractTableInfo($: ReturnType<typeof cheerio.load>): { amount: string; organization: string; deadline: string } {
    let amount = '';
    let organization = '';
    let deadline = '';
    ...
    $('table tr').each((i, row) => {
      ...
      if (cells.length >= 2) {
        ...
        if (header.includes('上限') || header.includes('補助額') || header.includes('金額')) {
          amount += value + ' ';
        }
        
        if (header.includes('実施機関') || header.includes('主催') || header.includes('担当')) {
          organization = value;
        }
        
        if (header.includes('募集期間') || header.includes('申請期限') || header.includes('締切')) {
          deadline = value;
        }
      }
    });
  ...
  private parseAmountInfo(text: string): { subsidyRate: string; maxAmount: string; minAmount: string } {
    let subsidyRate = '';
    let maxAmount = '';
    let minAmount = '';
  ...
  private estimateCategories(name: string, summary: string): string[] {
    ...
    const categoryKeywords = {
      '交通・運輸': ['交通', '運輸', 'バス', '電車', '移動', '輸送', 'モビリティ'],
      '地域振興': ['地域', '振興', 'まちづくり', '地方創生', 'リデザイン', '空白', '過疎'],

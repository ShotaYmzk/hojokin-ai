...
const HOJYOKIN_PORTAL_TARGETS: RealScrapingTarget[] = [
  {
    ...
    selectors: {
      container:
        ".subsidy-card, .card, .list-item, .subsidy-item, .item-card, tr, .content-box",
  ...
  },
  {
    ...
    selectors: {
      container:
        ".detail-section, .info-section, .content-section, table tr, .field-row",
...
export class RealSubsidyScraper {
    ...
    try {
      ...
      if (containers.length > 0) {
        ...
        if (meaningfulContainers.length > 0) {
          ...
          console.log(
            `📋 意味のある要素にフィルタリング: ${containers.length} 件`,
          );
        }
      }

      console.log(
        `📦 使用するセレクタ: "${usedSelector}" (${containers.length} 件)`,
      );
      ...
      if (containers.length === 0) {
        ...
        for (const keyword of subsidyKeywords) {
          try {
            ...
            console.log(
              `🔍 "${keyword}" を含む要素: ${keywordElements.length} 件`,
            );
        ...
        for (const selector of specificSelectors) {
          try {
            ...
            if (elements.length > 0) {
              console.log(
                `🎯 特定セレクタ "${selector}": ${elements.length} 件`,
              );
              if (elements.length > containers.length) {
                containers = elements;
                usedSelector = selector;
      ...
      containers.each((index, element) => {
        try {
          ...
          if (subsidy) {
            ...
            scrapedCount++;
            console.log(
              `✅ [${index + 1}] ${subsidy.name.substring(0, 50)}...`,
            );
          }
        } catch (parseError) {
      ...
      if (subsidies.length > 0) {
        ...
        console.log(
          `💾 ${subsidies.length} 件のデータをデータベースに保存しました`,
        );
      } else {
  ...
  private parseRealSubsidyElement(
    ...
    if (
      ...
      $el.prop("tagName")?.toLowerCase() === "tr"
    ) {
      const cells = $el.find("td");
      const cellTexts: string[] = [];
    ...
    for (const selector of deadlineSelectors) {
      try {
        ...
        if (deadlineEl.length > 0) {
          ...
          if (deadlineText && deadlineText.match(/\d{4}|\d{1,2}月|\d{1,2}日/))
            break;
        }
      } catch (selectorError) {
        continue;
    ...
    if (!organization) {
      if (target.url.includes("pref_id=48")) {
        ...
        name.includes("国土交通省") ||
        summary.includes("国土交通省")
      ) {
        organization = "国土交通省";
      } else if (
        name.includes("経済産業省") ||
        summary.includes("経済産業省")
      ) {
        organization = "経済産業省";
      } else if (
        name.includes("厚生労働省") ||
        summary.includes("厚生労働省")
      ) {
        organization = "厚生労働省";
      } else {
    ...
    // 補助金額の抽出（強化版）
    const { subsidyRate, maxAmount, minAmount } = this.parseAmountInfo(
      summary + " " + amountText + " " + name,
    );
  ...
  private async debugHtmlStructure(
    $: ReturnType<typeof cheerio.load>,
    url: string,
  ): Promise<void> {
    console.log("\n🔬 HTML構造の詳細調査");
    ...
    for (let index = 0; index < subsidies.length; index++) {
      ...
      try {
        ...
        if (existing) {
          ...
          console.log(
            `🔄 [${index + 1}] 更新: ${subsidy.name.substring(0, 40)}...`,
          );
          ...
          console.log(
            `➕ [${index + 1}] 新規: ${subsidy.name.substring(0, 40)}...`,
          );
          console.log(`    組織: ${subsidy.organization}`);
          console.log(`    金額: ${subsidy.maxAmount || "未設定"}`);
          console.log(`    カテゴリ: ${subsidy.categories.join(", ")}`);

...
import { Search, Filter, Calendar, MapPin, Building2, Users, DollarSign } from 'lucide-react';
...
const CATEGORIES = [
  'IT導入', '設備投資', 'DX推進', '新事業開発', '販路拡大', 
  '人材育成', '環境・省エネ', '研究開発', '創業支援', 'その他'
];

const INDUSTRIES = [
  '製造業', '情報通信業', '建設業', '卸売業・小売業', '宿泊業・飲食サービス業',
  '運輸業・郵便業', '金融業・保険業', '不動産業', '専門・技術サービス業',
  '生活関連サービス業', '医療・福祉', '教育・学習支援業', 'その他'
];
...
export default function SubsidySearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (filters.industries.length > 0) searchParams.append('industries', filters.industries.join(','));
      if (filters.prefecture) searchParams.append('prefecture', filters.prefecture);
      if (filters.employeeCountRange) searchParams.append('employeeRange', filters.employeeCountRange);
      if (filters.capitalAmountRange) searchParams.append('capitalRange', filters.capitalAmountRange);
      if (filters.applicationDeadline) searchParams.append('deadline', filters.applicationDeadline);
      searchParams.append('sort', filters.sortBy);
      searchParams.append('order', filters.sortOrder);

      const response = await fetch(`/api/subsidies/search?${searchParams.toString()}`, {
        headers: {
  ...
  const getDummyResults = (): SubsidyResult[] => [
    {
      ...
      name: 'IT導入補助金2025',
      organization: '経済産業省',
      summary: 'ITツール導入による中小企業・小規模事業者の生産性向上を支援する補助金制度です。',
    ...
    },
    {
      ...
      summary: '革新的な製品・サービス開発や生産プロセス改善に必要な設備投資等を支援します。',
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">条件を指定して最適な補助金制度を見つけましょう</p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateFilter('employeeCountRange', e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateFilter('capitalAmountRange', e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500"
                >
                  <option value="">指定なし</option>
      ...
      <div className="space-y-4">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-gray-50 rounded-xl">
            ...
            <p className="text-sm text-gray-500 mt-1">検索条件を変更してお試しください。</p>
          </div>
        ) : (
          <div className="grid gap-6">
            {results.map(result => (
              <div key={result.id} className="bg-white rounded-xl shadow-sm border p-6 hover:shadow-md transition-shadow">

...
const Button: React.FC<any> = ({
  ...
  return (
    ...
    >
      {isLoading ? (
        <Spinner className="mr-2" color="current" size="sm" />
      ) : null}
...
const Code: React.FC<any> = ({ children, className }) => (
  <pre
    className={`bg-content1 p-4 rounded-md overflow-x-auto text-sm whitespace-pre-wrap ${className}`}
  >
    {children}
  </pre>
);
...
export default function DocumentGenerationPage() {
  ...
  useEffect(() => {
    ...
    if (subsidyIdFromQuery) {
      const foundSubsidy = availableSubsidies.find(
        (s) => s.id === subsidyIdFromQuery,
      );
  ...
  const handleGenerateDocuments = async (event: FormEvent) => {
    ...
    if (!companyInfo || !selectedSubsidy || targetSections.length === 0) {
      setError(
        "企業情報、対象補助金、および生成するセクションを選択してください。",
      );
    ...
    try {
      ...
      const dummyData: GeneratedSection[] = targetSections.map((sectionKey) => {
        ...
        return {
          title: `${sectionKey} (AI案)`,
          content: `AIが${sectionKey}に関する内容を生成しました。\nこれはサンプルテキストです。`,
          editable: true,
        };
      });
  ...
  const handleEditGeneratedContent = (index: number, newContent: string) => {
    setGeneratedDocuments((prev) =>
      prev.map((doc, i) =>
        i === index ? { ...doc, content: newContent } : doc,
      ),
    );
  };
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      {error && (
        <div className="p-4 bg-danger-50 text-danger-700 rounded-md text-sm">
          {error}
        </div>
      )}
        ...
        <Card>
          <form onSubmit={handleGenerateDocuments}>
            ...
            <CardBody className="space-y-6">
              <div>
                <h3 className="text-md font-medium text-foreground-700 mb-1">
                  対象補助金
                </h3>
                <Select
                  required
                ...
                {!selectedSubsidy && availableSubsidies.length === 0 && (
              ...
              <div>
                <h3 className="text-md font-medium text-foreground-700 mb-2">
                  生成するセクションを選択
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {availableSections.map((section) => (
                    ...
                    >
                      ...
                      />
                      <span
                        className={`ml-3 text-sm font-medium ${targetSections.includes(section.key) ? "text-primary-700" : "text-foreground-700"}`}
                      >
                        {section.label}
                      </span>
                    </label>
              ...
              {companyInfo && (
                <div className="bg-content1 p-4 rounded-lg border border-divider">
                  ...
                  <p className="text-sm text-foreground-600">
                    事業内容:{" "}
                    {companyInfo.businessDescription.substring(0, 100)}...
                  </p>
                  <Link
      ...
      {currentStage === "generating" && (
        <Card className="text-center py-12">
          <CardBody className="flex flex-col items-center justify-center">
            ...
            <p className="mt-4 text-lg font-medium text-foreground-600">
              AIが書類ドラフトを生成中です...
            </p>
            <p className="text-sm text-foreground-500">
              通常10秒〜30秒程度かかります。しばらくお待ちください。
            </p>
          </CardBody>
        </Card>
      )}
        ...
        <div className="space-y-6">
                ...
                {doc.editable ? (
                  <Textarea
                    ...
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                      handleEditGeneratedContent(index, e.target.value)
                    }
                  />
                ) : (
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-8 pt-6 border-t border-divider">
            ...
            <Button
              className="w-full sm:w-auto"
              color="success"
              onClick={handleProceedToFullForm}
            >
      ...
      {currentStage === "results" &&
        ...
        !isLoading && (
          <Card className="text-center py-10">
            <CardBody>
              <p className="text-lg text-warning-700">
                ドラフトの生成に失敗したか、対象データがありませんでした。
              </p>
              <Button
                className="mt-4"

...
const dummySubsidyData: { [key: string]: SubsidyInfo } = {
  "1": {
    id: "1",
    name: "IT導入補助金2025",
    summary: "ITツール導入による生産性向上を支援する補助金制度です。",
    deadline: "2025-06-30",
    subsidyRate: "1/2〜2/3",
    maxAmount: "450万円",
  },
  "2": {
    id: "2",
    name: "小規模事業者持続化補助金",
    summary: "小規模事業者の販路開拓等の取り組みを支援します。",
    deadline: "2025-08-10",
    subsidyRate: "2/3",
    maxAmount: "50万円",
  },
...
const dummyApplicationData = {
  formData: {
    ...
    businessTitle: "クラウドERP導入による業務改革プロジェクト",
    businessSummary:
      "既存のオンプレミス会計システムをクラウドERPに移行し、全社の業務効率を20%向上させる。",
    totalAmount: "3000000",
    subsidyRequestAmount: "1500000",
  },
  tasks: [
    {
      id: "t1",
      label: "基本情報の入力",
      completed: true,
      relatedField: "companyName",
    },
    {
      id: "t2",
      label: "事業計画の入力",
      completed: true,
      relatedField: "businessTitle",
    },
    {
      id: "t3",
      label: "資金計画の入力",
      completed: true,
      relatedField: "totalAmount",
    },
    {
      id: "t4",
      label: "事業計画書の添付",
      completed: false,
      relatedField: "businessPlan",
    },
    {
      id: "t5",
      label: "見積書の添付",
      completed: false,
      relatedField: "quotations",
    },
    {
      id: "t6",
      label: "決算書の添付",
      completed: false,
      relatedField: "financialStatements",
    },
  ],
  uploadedFiles: {
    businessPlan: null,
...
const CardHeader: React.FC<any> = ({ children, className }) => (
  <div className={`p-4 sm:p-6 border-b border-divider ${className}`}>
    {children}
  </div>
);
...
const Button: React.FC<any> = ({
  ...
  return (
    <button
      className={`px-4 py-2 rounded-md font-medium transition-colors ${colorClasses} ${disabled ? "opacity-50 cursor-not-allowed" : ""} ${className}`}
...
const DocumentCreatePage: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="text-center py-20">申請情報を読み込んでいます...</div>
    );
  }
  if (!subsidy) {
    return (
      <div className="text-center py-20">
        <h2 className="text-xl text-foreground-700">
          補助金情報が見つかりません。
        </h2>
        ...
        <Link
          className="mt-6 inline-block bg-primary text-primary-foreground px-6 py-2 rounded-lg"
  ...
  return (
    <div className="space-y-8">
      <header>
        <Link
          className="text-sm text-foreground-500 hover:text-foreground-800 mb-2 inline-block"
          href="/dashboard"
        >
          ← ダッシュボードに戻る
        </Link>
      ...
      <Card>
        <CardHeader>
          <h2 className="text-xl font-semibold">申請進捗</h2>
        </CardHeader>
        <CardBody>
          ...
          <div className="mt-6">
            <h3 className="text-md font-semibold mb-3">残りのタスク</h3>
            <ul className="space-y-2">
              {tasks
                .filter((t) => !t.completed)
                .map((task) => (
                  <li key={task.id} className="text-sm text-foreground-600">
                    ・ {task.label}
                  </li>
                ))}
              {tasks.every((t) => t.completed) && (
                <li className="text-sm text-success-600">
                  すべてのタスクが完了しました！
                </li>
              )}
            </ul>
      ...
      <form className="space-y-8" onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            <h2 className="text-xl font-semibold">申請内容</h2>
          </CardHeader>
          <CardBody className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2 font-semibold text-foreground-700 border-b pb-2 mb-2">
              基本情報
            </div>
            <Input
              required
            ...
            <Input
              ...
              onChange={handleInputChange}
            />
            <div className="md:col-span-2 font-semibold text-foreground-700 border-b pb-2 mb-2 mt-4">
              事業計画
            </div>
            <Input
              required
            ...
            <Input
              required
              label="補助金申請額（円）"
              name="subsidyRequestAmount"
              type="number"
              value={formData.subsidyRequestAmount}
              onChange={handleInputChange}
            />
          </CardBody>
        </Card>
        ...
        <Card>
          <CardHeader>
            <h2 className="text-xl font-semibold">添付書類</h2>
          </CardHeader>
          <CardBody className="space-y-4">
            {/* ▼▼▼ 修正箇所: FileUploadにrefと削除ハンドラを渡す ▼▼▼ */}
            <FileUpload
              ref={fileInputRefs.businessPlan}
              file={uploadedFiles.businessPlan}
              label="事業計画書"
              name="businessPlan"
            ...
            />
            <FileUpload
              ref={fileInputRefs.quotations}
              file={uploadedFiles.quotations}
              label="見積書"
              name="quotations"
              onChange={handleFileUpload}
              onDelete={handleFileDelete}
            />
            <FileUpload
              ref={fileInputRefs.financialStatements}
              file={uploadedFiles.financialStatements}
              label="決算書（直近2期分）"
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button
            ...
            title={progress < 100 ? "すべてのタスクを完了してください" : ""}
            type="submit"
          >
            申請を提出する
          </Button>
...
const FileUpload = React.forwardRef<
  HTMLInputElement,
  {
    ...
    onDelete: (name: keyof UploadedFiles) => void;
  }
        ...
        {file ? (
          <div className="flex items-center justify-between p-3 border border-default-300 rounded-lg bg-content2">
            <div className="flex items-center overflow-hidden">
              <svg
                className="h-6 w-6 text-success-500 mr-3 flex-shrink-0"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
            ...
            <button
              className="text-foreground-400 hover:text-danger-500 ml-4 flex-shrink-0"
              type="button"
          ...
          <div className="flex justify-center px-6 pt-5 pb-6 border-2 border-default-300 border-dashed rounded-md">
            <div className="space-y-1 text-center">
              <svg
                aria-hidden="true"
                className="mx-auto h-12 w-12 text-foreground-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 48 48"
              >
              ...
              <div className="flex text-sm text-foreground-600">
                <label
                  className="relative cursor-pointer bg-background rounded-md font-medium text-primary hover:text-primary-focus focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary"
                  htmlFor={String(name)}
                >

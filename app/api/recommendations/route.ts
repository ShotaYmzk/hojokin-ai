...
export async function GET(request: NextRequest) {
  try {
    ...
    if (recommendations.length > 0) {
      ...
      await supabase.from("recommendations").insert(recommendationRecords);
    }
...
function calculateMatchingScore(
  ...
  if (subsidy.industries && subsidy.industries.length > 0) {
    ...
    if (subsidy.industries.includes(company.industry)) {
      ...
      subsidy.industries.includes("全業種") ||
      subsidy.industries.includes("すべて")
    ) {
      score += 2;
    }
  }
  ...
  // 従業員数マッチング（重要度: 中）
  if (
    subsidy.employeeCountMin !== undefined ||
    subsidy.employeeCountMax !== undefined
  ) {
    totalFactors += 2;
    const minOk =
      !subsidy.employeeCountMin ||
      company.employeeCount >= subsidy.employeeCountMin;
    const maxOk =
      !subsidy.employeeCountMax ||
      company.employeeCount <= subsidy.employeeCountMax;
  ...
  // 資本金マッチング（重要度: 中）
  if (
    subsidy.capitalAmountMin !== undefined ||
    subsidy.capitalAmountMax !== undefined
  ) {
    totalFactors += 2;
    const minOk =
      !subsidy.capitalAmountMin ||
      company.capitalAmount >= subsidy.capitalAmountMin;
    const maxOk =
      !subsidy.capitalAmountMax ||
      company.capitalAmount <= subsidy.capitalAmountMax;
  ...
  if (subsidy.prefecture) {
    totalFactors += 1;
    if (
      subsidy.prefecture === company.prefecture ||
      subsidy.prefecture === "全国"
    ) {
      score += 1;
    }
  }
  ...
  if (company.annualRevenue) {
    totalFactors += 1;
    // 売上規模に応じた補正
    if (company.annualRevenue <= 100000000) {
      // 1億円以下
      if (subsidy.name.includes("小規模") || subsidy.name.includes("創業")) {
        score += 1;
      }
    } else if (company.annualRevenue <= 1000000000) {
      // 10億円以下
...
function generateRecommendationReasoning(
  ...
  if (
    ...
    if (minOk && maxOk) {
      reasons.push(
        `貴社の従業員数（${company.employeeCount}名）が対象規模に適合しています`,
      );
    }
  }

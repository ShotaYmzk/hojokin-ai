...
export async function POST(request: Request) {
  try {
    ...
    const { email, password, companyName, companyAddress, representativeName } =
      body;
    ...
    if (companyName) {
      ...
      if (companyError) {
        ...
        try {
          ...
          console.log(
            `Cleaned up auth user due to company registration failure: ${userId}`,
          );
        } catch (deleteError) {
          console.error(
            `CRITICAL: Failed to clean up auth user ${userId}:`,
            deleteError,
          );
        }
